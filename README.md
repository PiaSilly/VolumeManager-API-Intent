...
The app exposes an exported broadcast receiver for automation apps.

**Action**
`moe.chensi.volume.action.SET_APP_VOLUME`

**Extras**
- `moe.chensi.volume.extra.PACKAGE` — target package name
- `moe.chensi.volume.extra.VOLUME` — volume multiplier as a `Float` from `0.0` (mute) to `1.0` (100%)

For example, from ADB:

```bash
adb shell am broadcast \
  -a moe.chensi.volume.action.SET_APP_VOLUME \
  -e moe.chensi.volume.extra.PACKAGE com.example.app \
  --ef moe.chensi.volume.extra.VOLUME 0.50
```

This sets `com.example.app` to 50% of its normal playback volume. The value is also persisted, so it is applied to future playback sessions of that package.

Automation apps can send the same broadcast using the action and extras above.
